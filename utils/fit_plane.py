...
from utils.error_funcs import ransac_error
...
def fit_plane(points: np.ndarray,
    ...
    while pow((1-pow(eps, m)),k) >= eta_0:
        ...
        error, inliers = error_func(points, distances, inlier_threshold)
        if error < error_star:
            I = np.sum(inliers)
            eps = I/N
...
def filter_planes(points: np.ndarray,
    ...
    plane_points = []
    for i in range(4):
        best_plane, best_inliers, num_iterations = fit_plane(remaining_points, confidence, inlier_threshold, min_sample_distance, error_func)
        if np.sum(best_inliers)/N <= min_points_prop:
            break
        plane_eqs.append(best_plane)
        plane_points.append(remaining_points[np.nonzero(best_inliers)[0]])
        best_inliers_inverted = np.invert(best_inliers)
        remaining_points = remaining_points[np.nonzero(best_inliers_inverted)[0]]

    return plane_eqs, plane_points, remaining_points

def extract_planes(points):
