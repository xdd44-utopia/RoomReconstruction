...
if __name__ == "__main__":
	...
	vertices.sort(key = lambda v: v.x)
	resultVertices = [vertices[0]]
	vertices.pop(0)
	vertices.sort(key = lambda v: angleLineAxis(resultVertices[0], v))
	resultVertices.append(vertices[0])
	cur = vertices.pop(0)
	vertices.append(resultVertices[0])

	# for ite in tqdm(range(len(vertices))):
	while (cur != resultVertices[0]):
		vertices.sort(key = lambda v: angleTwoVectors(resultVertices[-2], resultVertices[-1], resultVertices[-1], v))
		for i in range(len(vertices)):
			if (resultVertices[-1].distance(vertices[i]) < 0.1 and angleTwoVectors(resultVertices[-2], resultVertices[-1], resultVertices[-1], vertices[i]) > - math.pi / 2):
				cur = vertices.pop(i)
				resultVertices.append(cur)
				break
		if (cur == resultVertices[0]):
			break
	...
	for v in resultVertices:
		centroid.x += v.x
		centroid.y += v.y
	centroid.x /= len(resultVertices)
	centroid.y /= len(resultVertices)

	resultOBJ = NakedOBJ()
	resultVertices.append(centroid)
	resultFaces = []
	for i in range(1, len(resultVertices) - 1):
		resultFaces.append([len(resultVertices) - 1, i - 1, i])
	
	resultOBJ.vertices = [[v.x, v.y, 0] for v in resultVertices]
	resultOBJ.faces = resultFaces
	resultOBJ.export("testResult.obj")

	x = np.array([v.x for v in resultVertices])
	y = np.array([v.y for v in resultVertices])
	plt.plot(x, y)
	plt.show()
